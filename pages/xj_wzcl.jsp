...
<html>
	...
	<h3 class="text-primary">校级管理员${RT.name}</h3>
	</a>
	...
	<h6 class="mb-4">楼宇物资申请</h6>
	<div class="table-responsive">
	<table class="table">
	<thead>
	<tr>
	<th scope="col">区号</th>
	<th scope="col">面包数量</th>
	<th scope="col">香肠数量</th>
	<th scope="col">汽水数量</th>
	<th scope="col">泡面数量</th>
	<th scope="col">日期</th>
	<th scope="col">状态</th>
	<th scope="col">操作</th>
	</tr>
	...
	<td><c:choose >
	<c:when test="${li.statu==0}">
	<button onclick="wzpass(${li.sid})">通过</button>
	
	<button onclick="wzfail(${li.sid})">驳回</button>
	</c:when>
	<c:when test="${li.statu==1}">
	<button onclick="shopbuy(${li.sid})">超市采购</button>
	<button onclick="outterbuy(${li.sid})">校外采购</button>
	</c:when>
	</c:choose>
	...
	function shopbuy(sid){
		  ...
		  $.ajax({
	          ...
	          success: function(data) {
	        	  ...
	        	  window.location.href="tzshop.html";
	...
	function outterbuy(sid){
		  
		  $.ajax({
	          url: "/DM/outterbuy.html",
	          data: {"sid":sid},
	          type: "GET",
	          datatype:"text",
	          success: function(data) {
	          	
	        	  window.location.href="tzoutter.html";
