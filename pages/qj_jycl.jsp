...
<html>
	...
	<td><c:choose >
	<c:when test="${li.statu==1}">
	<button onclick="jypass(${li.sid})">通过</button>
	
	<button onclick="jyfail(${li.sid})">驳回</button>
	</c:when>
	<c:when test="${li.statu==4}">
	<button onclick="hospital(${li.sid})">找医生</button>
	</c:when>
	</c:choose>
	...
	function hospital(sid){
		
		 $.ajax({
	          url: "/DM/hospital.html",
	          data: {"sid":sid},
	          type: "GET",
	          datatype:"text",
	          success: function(data) {
	        	  window.location.href="hospitaltz.html";
