...
<html>
	...
	<td><c:choose >
	<c:when test="${li.statu==4}">
	<button onclick="bxpass(${li.sid})">通过</button>
	
	<button onclick="bxfail(${li.sid})">驳回</button>
	</c:when>
	<c:when test="${li.statu==5}">
	<button ><a onclick="bxy(${li.sid})">保修员联系</a></button><!-- 报修终结点 -->
	</c:when>
	</c:choose>
	...
	function bxy(sid){
		 ...
		 $.ajax({
	          ...
	          datatype:"text",
	          success: function(data) {
	        	  window.location.href="bxytz.html";
