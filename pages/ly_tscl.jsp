...
<html>
	...
	<td><c:choose >
	<c:when test="${li.statu==0}">
	<button onclick="tspass(${li.sid})">通过</button>
	
	<button onclick="tsfail(${li.sid})">驳回</button>
	</c:when>
	<c:when test="${li.statu==1}">
	<button onclick="tsbl(${li.snum},${li.sid})">办理</button><!-- 退宿终结点 -->
	</c:when>
	...
	function tsbl(snum,sid){
		
		 $.ajax({
	          url: "/DM/tsbl.html",
	          data: {"tssid":sid,"snum":snum},
	          type: "GET",
	          datatype:"text",
	          success: function(data) {
	        	  alert("退宿办理完成");
