...
<html>
	...
	<h3 class="text-primary">用户信息修改</h3>
	</a>
	...
	<a href="signin.html" class="dropdown-item">退出</a>
		...
		<h6 class="mb-4">用户信息修改</h6>
		
		<form action="" method="post" id="form_lssubmit">
		<div class="mb-3">
		<span>帐号:${user.account}</span>
		</div>
		<br>
		<div class="mb-3">
		<label >密码</label>
		<input style="width:400px;" type="text" id="password" name="pawd" class="form-control" value="${user.password}"/>
		</div>
		<div class="mb-3">
	    <label >姓名</label>
		<input style="width:400px;" type="text" class="form-control" id="name" name="name"  value="${user.name}"/>
		</div>
		
		<label for="exampleInputEmail1" class="form-label">电话</label>
		<input style="width:400px;" type="text"  class="form-control" id="tele" name="tele"  value="${user.tele}"/>
		
		<br>
		<div class="input-group">
		<button style='border:none;color:#0000EE;' onclick="userupdateinfo(${user.account})">确认提交</button>
		</div>
		</form>
		</div>
	...
	<script type="text/javascript">
      ...
      function userupdateinfo(account){
 		 ...
 		 $.ajax({
 	          url: "/DM/userupdate.html",
 	          data:{"password":password,"name":name,"tele":tele,"account":account},
 	          type: "GET",
 	          datatype:"text",
