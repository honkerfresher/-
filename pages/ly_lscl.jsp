...
<html>
	...
	<th scope="col">离宿日期</th>
	<th scope="col">留宿原因</th>
	<th scope="col">状态</th>
	<th scope="col">操作</th>
	</tr>
	</thead>
	...
	<c:choose >
	<c:when test="${li.statu==0}">
	<button onclick="lspass(${li.sid})">通过</button>
	
	<button onclick="lsfail(${li.sid})">驳回</button>
	</c:when>
	<c:when test="${li.statu==1}">
	<button onclick="qsfp(${li.snum},${RT.account},${li.sid})">分配</button><!-- 留宿终结点 -->
	</c:when>
	<c:when test="${li.statu==4}">
	...
	function qsfp(snum,account,lssid){
		
		 $.ajax({
	          url: "/DM/qsfp.html",//寝室分配
	          data: {"snum":snum},
	          type: "GET",
	          datatype:"text",
	          success: function(data) {
	        	  window.location.href="qsfptz.html?account="+account+"&lssid="+lssid;
