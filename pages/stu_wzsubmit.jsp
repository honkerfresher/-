...
<html>
	...
	<a href="signin.jsp" class="dropdown-item">退出</a>
		...
		<h6 class="mb-4">物资填报</h6>
		
		<form action="wzsubmit.html" method="post" id="form_wzsubmit">
		<div class="mb-3">
		...
		<span class="input-group-text">物资选择</span>
		<input type="text" style="width:50px" class="form-control" id="content" name="content" placeholder="物品以及数量" list="wzlist"  />
		<datalist id="wzlist">
								<c:forEach items="${wzlist}" var="li">
								<option value="${li.name}">${li.name}</option><!-- 保证数据无多余空格 -->
								</c:forEach>
								</datalist>
								<br> <br>
		</div>
		...
		<input type="text" size="5" class="form-control" id="amount" name="amount" placeholder="物品以及数量" list="wzamount"  />
		<datalist id="wzamount">
								
								<option value="1">1</option><!-- 保证数据无多余空格 -->
