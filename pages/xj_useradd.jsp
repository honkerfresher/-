...
<html>
	...
	<a href="signin.html" class="dropdown-item">退出</a>
		...
		<input type="text" class="form-control" id="account" name="account"  value="${stu.rmname}"/>
		</div>
		<div class="mb-3">
		<label for="exampleInputEmail1" class="form-label">密码</label>
		<input type="text"  class="form-control" id="password" name="password"  value="${stu.majo}"/>
		</div>
		<div class="mb-3">
		...
		<button style='border:none;color:#0000EE;' onclick="useraddinfo()">确认提交</button>
		</div>
		</form>
	...
	<script type="text/javascript">
      ...
      function useraddinfo(){
 		 ...
 		 $.ajax({
 	          url: "/DM/useraddinfo.html",
 	          data:{"account":account,"name":name,"password":password,"tele":tele},
 	          type: "GET",
 	          datatype:"text",
